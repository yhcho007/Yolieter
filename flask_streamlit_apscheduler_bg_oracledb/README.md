...
CREATE TABLE task (
    ...
    taskreserveid NUMBER,
    taskname VARCHAR2(100) NOT NULL,
    subprocee_starttime TIMESTAMP NOT NULL,
    task_status VARCHAR2(20) DEFAULT 'R' NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    lastchanged_at TIMESTAMP,
    CONSTRAINT fk_taskreserve foreign key(taskreserveid) references taskreserve(taskreserveid)
);

CREATE TABLE taskreserve (
    taskreserveid NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    taskname VARCHAR2(100) NOT NULL,
    start_date DATE,
    end_date DATE,
    start_time VARCHAR(20) DEFAULT '08:00:00',
    frequency VARCHAR(20),
    specific_months VARCHAR(40),
    specific_weekdays VARCHAR(40),
    specific_days_of_month VARCHAR(40),
    use_yn VARCHAR2(5) DEFAULT 'Y' NOT NULL,
    register VARCHAR(40) NOT NULL,
    lastchanger  VARCHAR(40) NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    lastchanged_at TIMESTAMP 
);
COMMENT ON COLUMN taskreserve.frequency IS 'daily, weekly, montly peak 1, NULL is every daily';
COMMENT ON COLUMN taskreserve.specific_months IS '1~12 NULL is every month';
COMMENT ON COLUMN taskreserve.specific_weekdays IS '0 is Monday, 6 is Sunday, NULL is every month';
COMMENT ON COLUMN taskreserve.specific_days_of_month IS '1~31 NULL is every day';


CREATE TABLE taskreservelog (
    taskreservelogtime TIMESTAMP,
    taskreserveid NUMBER,
    taskname VARCHAR2(100) NOT NULL,
    start_date DATE,
    end_date DATE,
    ...
    specific_days_of_month VARCHAR(40),
    use_yn VARCHAR2(5) DEFAULT 'Y' NOT NULL,
    status VARCHAR2(5) DEFAULT 'C' NOT NULL,
    lastchanger  VARCHAR(40) NOT NULL
);
